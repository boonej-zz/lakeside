<?php
/** A simple text block **/
class ML_MP_Services extends ML_Block {
	
	//set and create block
	function __construct() {
		$block_options = array(
			'name' => esc_html__('Services','association'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('ML_MP_Services', $block_options);
	}
	
	function form($instance) {
                
	$defaults = array('title' => '','subtitle' => '', 'post_type' => 'all', 'categories' => '', 'posts' => 3,'type_sel' =>'','layout_sel' =>'','columns_sel' => '3',
	'block_bg_color' => '#f9f9f9','block_text_color' => '#000','height' => '60' );
	...
	$layout_type = array(
				'' => esc_html__('Default','association'),
				'modern' => esc_html__('Modern','association'),
			);
	...
	$columns_type = array(
				...
				'5' => esc_html__('5 Columns','association'),
			);
	...
	extract($instance); ?>		
        ...
        <div class="block_bg"  style="background-color:<?php echo esc_attr($block_bg_color);?>;"></div>
            ...
            <?php echo do_shortcode('[mp-services category="'.  esc_attr($categories) .'" type="'. esc_attr($type).'" layout="'. esc_attr($layout).'" columns="'. esc_attr($columns).'"  posts="'. esc_attr($posts).'"]'); ?>   
            </div> 
